...
for ss, spec2D_file in enumerate(spec2D_all):
    ...
    for i in range(len(stars)):
        ...
        # Get 1D spectrum
        spec1D_star_eachframe, FWHM_pix = bt.extract1D_psf(spec2D_star, psf_lim=0.02, plot_psf=False, ax=None)
        print("1dStar after extraction:", np.nanmedian(spec1D_star_eachframe))
        ...
        plt.plot(spec1D_star_eachframe*fluxcal.smooth_calibration(wave_A, calibration, degree=smooth_calib_order))
        plt.show()

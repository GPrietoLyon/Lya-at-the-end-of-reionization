...
def FitPowerLaw(photometry,filters,Cwaves,ShowPlots=True):
    ...
    Data = np.array([photometry[filter][0] for filter in filters])
    Mags=np.array(Data[:int(len(Data)/2)])
    MagsErr=np.array(Data[int(len(Data)/2):])
    #print(Mags)
    ...
    logFerr[np.isnan(logFerr)==True]=0.3

    # Mask out NaN values in fluxes
    valid_mask = ~np.isnan(fluxes)
    fluxes = fluxes[valid_mask]
    logFerr = logFerr[valid_mask]
    Cwaves = Cwaves[valid_mask]

    
    #print(Cwaves,fluxes,logFerr)
    if len(fluxes) ==1:
        return np.nan, np.nan, [np.nan, np.nan], [np.nan, np.nan]

    popt, pcov = curve_fit(PowerLaw, np.log10(Cwaves), np.log10(fluxes), sigma=logFerr)
